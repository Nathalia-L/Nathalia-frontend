...
const glass = { background: '#241219', border: '1px solid rgba(255,255,255,0.08)' }
...
function ClienteInicio() {
  ...
  return (
    <div className="text-white" style={{ background: '#1A0E13' }}>
      {/* HERO DE BIENVENIDA */}
      <section className="relative overflow-hidden min-h-[520px] flex items-end">
        ...
        <AuroraBackground />
        <div className="absolute inset-0 bg-gradient-to-t from-[#1A0E13] via-[#1A0E13]/60 to-[#1A0E13]/20"></div>
        <div className="absolute inset-0 bg-gradient-to-r from-[#1A0E13]/50 via-transparent to-transparent"></div>

        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-24 pb-12 sm:pb-16 w-full">
          <div className="flex items-center gap-2 mb-4">
            <span className="w-1.5 h-1.5 rounded-full bg-[#C77A9C]"></span>
          ...
          <h1 className="text-3xl sm:text-5xl font-medium leading-tight tracking-tight text-white max-w-xl">
            {cliente.nombre ? <>Hola, {cliente.nombre} ☕</> : 'Bienvenido a Nathalia'}
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-8">
            <button
              ...
              className="px-6 sm:px-8 py-3 sm:py-3.5 bg-[#C77A9C] text-white rounded-xl font-medium text-sm hover:bg-[#A65E80] transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C77A9C] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1A0E13]"
            >
              Ir al catálogo →
            </button>
          </div>
          ...
          {VALORES.map((v, i) => (
            <FadeIn key={v.titulo} delay={i * 0.1}>
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-[#EBC6D6]" style={{ background: 'rgba(199,122,156,0.15)' }}>
                  {v.icono}
                </div>
                <div>
      ...
      <FadeIn>
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-14 sm:py-20">
          <div className="flex items-center justify-between mb-6">
            <div>
              <span className="text-xs font-medium text-[#EBC6D6] uppercase tracking-wide">Del catálogo</span>
            ...
            {destacados.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    <p className="text-[10px] font-medium text-[#EBC6D6] uppercase tracking-wide">{esMaquina ? 'Máquina' : 'Café'}</p>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-xs font-medium text-[#EBC6D6] group-hover:underline">Ver →</span>
                    </div>
                  </div>
                </div>
              )
          ...
          <div className="anim-pop rounded-2xl overflow-hidden divide-y sm:divide-y-0 sm:divide-x sm:grid sm:grid-cols-3" style={{ ...glass, borderColor: 'rgba(255,255,255,0.1)' }}>
            {ACCESOS.map((a) => (
              ...
              >
                ...
                <span className="text-white/30 group-hover:text-[#EBC6D6] group-hover:translate-x-0.5 transition-all">→</span>
              </button>
            ))}
          </div>
        </section>
