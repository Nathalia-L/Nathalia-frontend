...
const rolLabel = { admin: 'Admin', gerente: 'Gerente' }
const rolBadge = {
...
function Inventario() {
  ...
  const [form, setForm] = useState({ nombre: '', apellido: '', email: '', contraseña: '', rol: 'gerente' })
  ...
  const crearUsuario = async (e) => {
    ...
    try {
      ...
      setShowModal(false)
      setForm({ nombre: '', apellido: '', email: '', contraseña: '', rol: 'gerente' })
    } catch (err) {
                ...
                <tr key={user.id_usuario} className="row-hover border-b border-[#eef4ef] transition-colors">
                  ...
                  <td className="px-6 py-3.5">
                    <button
                      ...
                      style={{ background: user.estado === 'activo' ? '#C77A9C' : '#d1d5db' }}
                    > 
                      <span
