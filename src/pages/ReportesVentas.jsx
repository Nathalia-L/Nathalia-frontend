...
function ReportesVentas() {
  ...
  const topProductos = datos?.top_productos.map((p, i) => ({
    ...
    color: ['bg-[#E8C786]', 'bg-[#C77A9C]', 'bg-[#2B1B12]', 'bg-[#D4AF37]', 'bg-[#EBC6D6]'][i] || 'bg-[#C77A9C]'
  })) || []

  const colores = ['#E8C786', '#C77A9C', '#2B1B12', '#D4AF37', '#EBC6D6']

  const pieData = datos?.top_productos.map((p, i) => ({
    name: p.nombre,
    value: Number(p.kg_vendidos),
    color: colores[i] || '#C77A9C'
  })) || []
        ...
        {PERIODOS.map((p) => (
          <button
            ...
            className={`text-sm px-4 py-1.5 rounded-lg transition ${
              periodoActivo === p.valor ? 'bg-[#2A1521] text-white' : 'text-gray-500 hover:bg-gray-100'
            }`}
          >
            {p.label}
          </button>
            ...
            {cargando ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={tendencia}>
                  ...
                  <Line type="monotone" dataKey="ventas" stroke="#C77A9C" strokeWidth={2} dot={{ fill: '#C77A9C', r: 4 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
