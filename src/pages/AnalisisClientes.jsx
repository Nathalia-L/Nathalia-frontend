...
function AnalisisClientes() {
 ...
 const coloresBadge = ['bg-[#2A1521] text-white', 'bg-green-100 text-green-700', 'bg-amber-100 text-amber-700', 'bg-blue-100 text-blue-700', 'bg-purple-100 text-purple-700']
 const badges = ['VIP', 'Frecuente', 'Potencial', 'Nuevo', 'Regular']
      ...
      const clientes = datos?.top_clientes.map((c, i) => ({
        ...
        barColor: ['#C77A9C', '#7CB342', '#D8932F', '#3B82F6', '#8B5CF6'][i] || '#C77A9C',
      })) || []
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {stat.change && (
              <p className={`text-xs mt-1 ${stat.tone === 'orange' ? 'text-[#D8932F]' : 'text-[#C77A9C]'}`}>
